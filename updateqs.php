...
echo "<script>alert(\"Database error retry after some time !\")</script>";
   ...
   // $qid = 1;
    ...
    if (isset($_POST['submit1'])) {
        ...
        //$op4 = $_POST["op4"];
        $ans = $_POST["ans"];
        $sql = "update staff set name='$op2',mail='$op3',phno='$ans' where staffid='{$staffid}'";
        $res =   mysqli_query($conn, $sql);
        //$res =   mysqli_query($conn, $sql);
        if ($res == true) {
            echo '<script>alert("Done updation");</script>';
          echo "<script>window.location.replace(\"staffprofile.php?staffid=".$qid."\")</script>";
